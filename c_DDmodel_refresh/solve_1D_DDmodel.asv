...
M = assemble_1D_matrix_from_integral(Gauss_coefficient,inv_a,h,mp,1,1);
[H1,R1,L1] = generate_1D_center_flux_local(inv_a,h,mp,1,0,cha);
...
while t_m < T_end
    ...
    % 使用RK方法求解该时间步下的结果
    % io = 1
    [phi,E] = solve_poisson_1D_FEM(mp,ng,P_partition,T_partition,Pb,Tb,Gauss_coefficient,h,mid_points,nh);
    for n = 2:ng+1
        % M2 = assemble_1D_coefficient_matrix_from_integral(Gauss_coefficient,inv_a,phi,Tb,h,mp,n-1,xmu);
        [H3,R3,L3] = generate_1D_coefficient_center_flux_local(Gauss_coefficient,inv_a,phi,E,Tb,mp,h,n,xmu);
        % Lh = L-L3;
        % Rh = R-R3;
        % Hh = H-H3;
...
t3 = toc
plot_phi_FEM(phi,P_partition,T_partition,Tb,basis_type);
