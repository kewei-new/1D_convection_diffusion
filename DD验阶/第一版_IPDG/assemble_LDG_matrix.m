function [LDG_matrix,f_values,Q_matrix]=assemble_LDG_matrix(inv_mass,ng,mp,mid_points,h,H,R,L,t)
...
b2 = zeros(ng*(mp+1),1);
n = 1;
g_a = boundary_fun(mid_points(n)-h/2,t);
for m = 0:mp
    b2((n-1)*(mp+1)+1+m,1) =-1/h*inv_mass(m+1)*g_a*reference_basis(-1/2,m);
end

n = ng;
g_b = boundary_fun(mid_points(n)+h/2,t);
for m = 0:mp
