...
b2 = zeros(ng*(mp+1),1);
n = 1;
g_a = LDG_bound(mid_points(n)-h(n)/2,t);
for m = 0:mp
    b2((n-1)*(mp+1)+1+m,1) =-1/h(n)*inv_mass(m+1)*g_a*reference_basis(-1/2,m);
end
n = ng;
g_b = LDG_bound(mid_points(n)+h(n)/2,t);
for m = 0:mp
    b2((n-1)*(mp+1)+1+m,1) = 1/h(n)*inv_mass(m+1)*g_b*reference_basis(1/2,m);
end
% 
b1 = zeros(ng*(mp+1),1);
n = ng;
...
for n = 1:ng
    ...
    for m = 0:mp
        f_values(m+1,n)=f_values(m+1,n)+Gw'*((n_value - f2).*reference_basis(Gp,m))*h(n);
    end
