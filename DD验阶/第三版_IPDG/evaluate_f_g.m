...
for k = 2:ng+1
   ...
   u1x(:,k) = evaluate_uhx_DG(Gp,uh1(:,k-1),mp)*h(k-1)^(-1);
   phix(:,k) = evaluate_uh_DG(Gp,phixh(:,k-1),mp);
end
...
for k = 2:ng+1
    for m = 0:mp
        ...
        f(m+1,k-1,1) = inv_mass(m+1)*(-Gw'*(f1(:,k).*reference_basis_x(Gp,m))...
...
for k = 2:ng+1
    for m = 0:mp
        ...
        % g(m+1,k-1,1) =inv_mass(m+1)*(-Gw'*(g1x(:,k).*reference_basis_x(Gp,m))...
        %       +1/2*((g1x(2,k)+g1x(1,k+1))*reference_basis(0.5,m)-(g1x(2,k-1)+g1x(1,k))*reference_basis(-0.5,m))...
        %       +1/2*((g1(2,k)-g1(1,k+1))*reference_basis_x(0.5,m)+(g1(2,k-1)-g1(1,k))*reference_basis_x(-0.5,m))*h(k-1)^(-1)*method...
        %       -C_div_h*((g1(2,k)-g1(1,k+1))*reference_basis(0.5,m)+(g1(1,k)-g1(2,k-1))*reference_basis(-0.5,m)))*h(k-1)^(-1);

            g(m+1,k-1,1) =inv_mass(m+1)*(-Gw'*(g1x(:,k).*reference_basis_x(Gp,m))...
          +1/2*((g1x(2,k)+g1x(1,k+1))*reference_basis(0.5,m)-(g1x(2,k-1)+g1x(1,k))*reference_basis(-0.5,m))...
          +1/2*((g1(2,k)-g1(1,k+1))*reference_basis_x(0.5,m)+(g1(2,k-1)-g1(1,k))*reference_basis_x(-0.5,m))*h(k-1)^(-1)*method...
          -C_div_h/h(k-1)*((g1(2,k)-g1(1,k+1))*reference_basis(0.5,m)+(g1(1,k)-g1(2,k-1))*reference_basis(-0.5,m)))*h(k-1)^(-1);
