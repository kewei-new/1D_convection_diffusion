function [uh,qh]=solve_1D_LDG(Gauss_coefficient,inv_mass,h,mid_points,mp,ng,uh1,t)
...
for n = 1:ng
    ...
    if n == 1        
        ...
        for alpha = 0:mp %test
            ...
            b1(alpha+1) =-inv_mass(alpha+1)*h(n)^(-1)*LDG_bound(mid_points(1)-h(1)/2,t)*reference_basis(-1/2,alpha);
        end   
    ...
    elseif n <ng
        ...
        for alpha = 0:mp %test
            ...
            b1((n-1)*(mp+1)+(alpha+1)) = inv_mass(alpha+1)*h(n)^(-1)*LDG_bound(mid_points(ng)+h(ng)/2,t)*reference_basis(1/2,alpha);
        end 
    end
...
for n = 1:ng
    ...
    if n < ng
        ...
        for alpha = 0:mp %test
           ...
           b2((n-1)*(mp+1)+(alpha+1)) = C_p*LDG_bound(mid_points(ng)+h(ng)/2,t)*reference_basis(1/2,alpha); 
        end              
    end
end
...
for n = 1:ng
    ...
    for m = 0:mp
        f_values = f_values + uh1(m+1,n)*reference_basis(Gp,m);
    end

    for alpha = 0:mp
        f_terms((n-1)*(mp+1)+(alpha+1),1) = Gw'*((f_values - f2_fun(Gp_lcoal,t)).*reference_basis(Gp,alpha))*h(n);
    end
end
