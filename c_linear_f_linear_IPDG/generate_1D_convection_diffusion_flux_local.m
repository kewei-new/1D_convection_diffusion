...
for n = 2:ng-1
    ...
    elseif term_type==12
        H(:,:,n) = inv_matrix_A*(1/2*(matrix_E(:,:,1)-matrix_E(:,:,2))/h_local(n))/h_local(n);
        R(:,:,n+1) = inv_matrix_A*(1/2*matrix_E(:,:,4)/h_local(n+1))/h_local(n);
        L(:,:,n-1) = inv_matrix_A*(-1/2*matrix_E(:,:,3)/h_local(n-1))/h_local(n);
    end
...
if term_type==11
    H(:,:,n) = 1/2*inv_matrix_A*(matrix_D(:,:,1)-matrix_D(:,:,2))/h_local(n);
    R(:,:,n+1) = 1/2*inv_matrix_A*matrix_D(:,:,4)/h_local(n);
    L(:,:,ng) = -1/2*inv_matrix_A*matrix_D(:,:,3)/h_local(n);
elseif term_type==12
    H(:,:,n) = inv_matrix_A*(1/2*(matrix_E(:,:,1)-matrix_E(:,:,2))/h_local(n))/h_local(n);
    R(:,:,n+1) = inv_matrix_A*(1/2*matrix_E(:,:,4)/h_local(n+1))/h_local(n);
    L(:,:,ng) = inv_matrix_A*(-1/2*matrix_E(:,:,3)/h_local(ng))/h_local(n);
end
% N
n=ng;
if term_type==11
    H(:,:,n) = 1/2*inv_matrix_A*(matrix_D(:,:,1)-matrix_D(:,:,2))/h_local(n);
    R(:,:,1) = 1/2*inv_matrix_A*matrix_D(:,:,4)/h_local(n);
    L(:,:,n-1) = -1/2*inv_matrix_A*matrix_D(:,:,3)/h_local(n);
elseif term_type==12
    H(:,:,n) = inv_matrix_A*(1/2*(matrix_E(:,:,1)-matrix_E(:,:,2))/h_local(n))/h_local(n);
    R(:,:,1) = inv_matrix_A*(1/2*matrix_E(:,:,4)/h_local(1))/h_local(n);
    L(:,:,n-1) = inv_matrix_A*(-1/2*matrix_E(:,:,3)/h_local(n-1))/h_local(n);
end
