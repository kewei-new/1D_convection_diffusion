classdef CaseRunner
    ...
    methods (Static, Access = private)
        function opts = parseOptions(varargin)
            opts = struct("elements", 16, "order", 2, "t_final", 0.01, "dt", 1.0e-3);
        ...
        function metrics = runSmoothMMS1D(opts)
            ...
            n_coeff = mfemdd.Coefficient(@(x) 1.0 + 0.2 .* sin(pi .* x(:,1)), "n_exact");
            phi_coeff = mfemdd.Coefficient(@(x) sin(2.0 .* pi .* x(:,1)), "phi_exact");
            ...
            metrics.charge_proxy = trapz(mesh.Nodes(:,1), n.Values - 1.0);
