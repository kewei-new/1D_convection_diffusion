function [t_m,n_last]=solve_1D_DDmodel(mo,k)
...
for k = 1:ng
    for m = 0:mp
        n_last(k) = n_last(k) + nh(m+1,k)*reference_basis(0,m);  
    end
end
...
while t_m < T_end && error>1e-6
    ...
    for k = 1:ng
        n_value = 0;
        for m = 0:mp
            n_value = n_value + nh(m+1,k)*reference_basis(0,m);  
        end
        error = error + abs(n_value - n_last(k));
        n_last(k) = n_value;
    end
    error = error/ng
